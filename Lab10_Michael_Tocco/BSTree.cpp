...
double BSTree<DataType, KeyType>::sumKeys() const
{
    return sumKeysHelper(root);
}
...
template<typename DataType, class KeyType>
double BSTree<DataType, KeyType>::sumKeysHelper(BSTreeNode* p) const
{
    if (p != 0)
    {
        return sumKeysHelper(p->left) + stoi(p->dataItem.getKey()) + sumKeysHelper(p->right);
    }
    return 0;
}

template<typename DataType, class KeyType>
double BSTree<DataType, KeyType>::getNumerator(double mean)
{
    return getNumeratorHelper(root, mean);
}

template<typename DataType, class KeyType>
double BSTree<DataType, KeyType>::getNumeratorHelper(BSTreeNode* p, double mean)
{
    if (p != 0)
    {
        return (getNumeratorHelper(p->left, mean) + ((stoi(p->dataItem.getKey()) - mean) * (stoi(p->dataItem.getKey()) - mean)) + getNumeratorHelper(p->right, mean));
    }
    else
        return 0;
}
